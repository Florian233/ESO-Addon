...
function SetLocker:FindBagSlotID(itemID, itemName)
   local bagSize = GetBagSize(BAG_BACKPACK)
   for i=1,bagSize,1 do
      local item = GetItemLink(BAG_BACKPACK, i)
      local id = GetItemLinkItemId(item)
      local name = GetItemLinkName(item)
...
function SetLocker.OnItemPickup(eventCode, lootedBy, itemLink, quantity, itemSound, lootType, isNotStolen)
  ...
  if SetLocker.playerName == lootedPlayer then
     if setName ~= "" and SetLocker.units[tostring(setName)] ~= nil and SetLocker.units[tostring(setName)].Locked == "Yes" then
         ...
         local slotID = SetLocker:FindBagSlotID(itemID, name)
         SetItemIsPlayerLocked(BAG_BACKPACK, slotID, true)
...
function SetLocker:Initialize()
  EVENT_MANAGER:RegisterForEvent(SetLocker.name, EVENT_LOOT_RECEIVED, SetLocker.OnItemPickup)
  SetLocker.GUIOpen = false
